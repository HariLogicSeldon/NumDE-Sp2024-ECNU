...
def main():
    ...
    step_sizes = np.array([1 * np.power(0.5, i) for i in range(6)])
    num_intervals = ((end - start) / step_sizes).astype(int)

    u_error = []
    t_fixed = np.linspace(start, end, 1000)
    u_true = true_u_func(t_fixed)
    plt.plot(t_fixed, u_true, label="U True")

    for n in range(4):
        alg = num_ode_lab1(start, end, num_intervals[n], u_start, f, true_u_func)
        t = alg.iter_point()
        u_euler = alg.two_step(-1)
        u_error.append(alg.error()[0])

        plt.scatter(t, u_euler, label=num_intervals[n], s=50 + n * 3, alpha=0.5)
